...
1. **Planilhas ODS** em `CLG/` — uma por mês (aba `Consumos` + aba `Estoques Físicos`).
   Extração: `python3 scripts/clg_extrair.py CLG` (JSON pronto para conferência).
2. **PMPE em PDF** (pasta a ser criada pelo usuário) — contém os **períodos de
   comissão** de cada navio. É de lá que saem os **dias de comissão** dos
   consumos operativos (a planilha não traz esse dado).
